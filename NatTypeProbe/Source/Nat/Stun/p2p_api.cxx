...
{ 
   ...
   bool isNat=true;
   //StunAddress4 testIchangedAddr; //not used
...
int stunPunch(StunAddress4& dst, bool verbose, StunAddress4* src)
{
    ...
    Socket myFd1 = openPort(src->port, src->addr, verbose);
    ...
    while (count < 100)
    {
        ...
        else if (err == 0)
        {
            // timeout occured
            count++;
            sendMessage(myFd1, (char*)"stun punch", strlen("stun punch"), dst.addr, dst.port, verbose);
        }
        else
        {
            if (myFd1 != INVALID_SOCKET)
            {
                if (FD_ISSET(myFd1, &fdSet))   //只要收到数据,说明对端udp洞已经打开
                {
                    char msg[100];
                    int msgLen = sizeof(msg);
